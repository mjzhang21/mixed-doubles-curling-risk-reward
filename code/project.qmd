...
hammer_counts = end_df[end_df['hammer']==1]['result'].value_counts().reindex(results, fill_value=0)
nonhammer_counts = end_df[end_df['hammer']==0]['result'].value_counts().reindex(results, fill_value=0)

x = np.arange(len(results))
width = 0.35  # width of the bars

fig, ax = plt.subplots(figsize=(8,6))
ax.bar(x - width/2, hammer_counts, width=width, label='Hammer', color='blue')
ax.bar(x + width/2, nonhammer_counts, width=width, label='Non-Hammer', color='orange')
...
def compute_end_features_from_coords(df):
    ...
    for (match_id, end_id), group in df.groupby(['match_id','end_id']):
        ...
        nh_stones_x = nh[['stone_2_x','stone_3_x','stone_4_x','stone_5_x']].values.flatten()
        nh_stones_y = nh[['stone_2_y','stone_3_y','stone_4_y','stone_5_y']].values.flatten()
        ...
        end_feat['nh_stones_in_house'] = np.sum(nh_dists <= 183)  # house radius ~183
        end_feat['nh_stones_out_of_play'] = np.sum((nh_stones_x == 0) | (nh_stones_y == 0) | (nh_stones_x == 4095) | (nh_stones_y == 4095))
        ...
        h_stones_x = h[['stone_8_x','stone_9_x','stone_10_x','stone_11_x']].values.flatten()
        h_stones_y = h[['stone_8_y','stone_9_y','stone_10_y','stone_11_y']].values.flatten()
        ...
        end_feat['h_stones_out_of_play'] = np.sum((h_stones_x == 0) | (h_stones_y == 0) | (h_stones_x == 4095) | (h_stones_y == 4095))
        ...
        end_feat['takeout_diff'] = end_feat['nh_takeout_count'] - end_feat['h_takeout_count']
        end_feat['draw_diff']    = end_feat['nh_draw_count'] - end_feat['h_draw_count']
        end_feat['guard_diff']   = end_feat['nh_guard_count'] - end_feat['h_guard_count']

        end_feat['takeout_guard_interaction'] = end_feat['takeout_diff'] * end_feat['guard_diff']
        ...
        if 'result' in group.columns:
            end_feat['steal'] = int(group.loc[group['team_id']==non_hammer_team, 'result'].iloc[0] > 0)

        # Distance differences
        end_feat['min_dist_diff'] = end_feat['nh_min_dist_button'] - end_feat['h_min_dist_button']
        end_feat['avg_dist_diff'] = end_feat['nh_avg_dist_button'] - end_feat['h_avg_dist_button']
        end_feat['max_dist_diff'] = end_feat['nh_max_dist_button'] - end_feat['h_max_dist_button']
        end_feat['std_dist_diff'] = end_feat['nh_std_dist_button'] - end_feat['h_std_dist_button']
...
for name, clf in classifiers.items():
    ...
    if name == 'Logistic L1':
        ...
        feat_df = pd.DataFrame({'feature': feature_cols, 'coef': coefs}).sort_values(
        ...
        feat_df = pd.DataFrame({'feature': feature_cols, 'importance': importances}).sort_values(
...
explainer = shap.Explainer(rf_model, X_train)  # X_train used as background data
shap_values = explainer(X_test)  # returns a structured object

# Summary plot for class 1 ("steal")
