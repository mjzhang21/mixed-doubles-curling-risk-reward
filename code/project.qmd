...
```
```{python}
num_stones = 12  # total stones per end
...
for i in range(1, num_stones + 1):
    ...
    # Create in-play flag
    df[f"stone_{i}_in_play"] = df[x_col].apply(lambda x: 0 if x == 4095 else 1)
    
    # Create thrown flag: 1 if thrown (x>0 and not 4095)
    if i in [1, 7]:  # preplaced stones
        df[f"stone_{i}_thrown"] = 0
    else:
        df[f"stone_{i}_thrown"] = df[x_col].apply(lambda x: 1 if 0 < x < 4095 else 0)
    
    # Preplaced stones
    df[f"stone_{i}_preplaced"] = 1 if i in [1, 7] else 0
    
    # Normalize x and y coordinates for stones that are in play
    df[f"stone_{i}_x_norm"] = df[x_col].apply(lambda x: x / 1500 if 0 < x < 4095 else 0)
    df[f"stone_{i}_y_norm"] = df[y_col].apply(lambda y: y / 3000 if 0 < y < 4095 else 0)
feature_cols = []
for i in range(1, num_stones + 1):
    feature_cols += [
        f"stone_{i}_x_norm",
        ...
        f"stone_{i}_in_play",
        f"stone_{i}_thrown",
        f"stone_{i}_preplaced"
    ]
feature_cols += ["powerplay","task"]
```
```{python}
import umap
X = df[feature_cols]
...
reducer = umap.UMAP(
    n_neighbors=60,    # controls local vs global structure
    min_dist=0.7,      # controls spacing of points
    n_components=2, 
    random_state=42
)
