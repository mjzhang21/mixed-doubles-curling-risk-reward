...
for i in range(1, num_stones + 1):
    ...
    new_cols[f"stone_{i}_in_play"] = ((x > 0) & (x < 4095)).astype(int)
    
    # Thrown flag
    if i in [1, 7]:
        new_cols[f"stone_{i}_thrown"] = 0
    else:
        new_cols[f"stone_{i}_thrown"] = ((x > 0) & (x < 4095)).astype(int)
    
    # Normalized coordinates
    new_cols[f"stone_{i}_x_norm"] = np.where((x > 0) & (x < 4095), x / 1500, 0)
    new_cols[f"stone_{i}_y_norm"] = np.where((y > 0) & (y < 4095), y / 3000, 0)
    
    # Radial distance and angle
    r = np.sqrt((x - button_x)**2 + (y - button_y)**2)
    new_cols[f"stone_{i}_r_norm"] = r / np.sqrt(1500**2 + 3000**2)
    new_cols[f"stone_{i}_theta"] = np.arctan2(y - button_y, x - button_x)
...
reducer = umap.UMAP(
    ...
    min_dist=0.9,      # controls spacing of points
    n_components=2, 
    random_state=42,
    low_memory=True,
)
...
plt.xlabel("UMAP 1")
plt.ylabel("UMAP 2")
plt.colorbar(label="Has Hammer")
plt.title("UMAP embedding of curling shot-level board states")
plt.show()
```
```{python}
plt.figure(figsize=(10,8))
plt.scatter(df["umap_x"], df["umap_y"], alpha=0.7, c=df["end_id"],cmap="viridis")
plt.xlabel("UMAP 1")
plt.ylabel("UMAP 2")
...
```{python}
plt.figure(figsize=(10,8))
plt.scatter(df["umap_x"], df["umap_y"], alpha=0.7, c=df["powerplay"],cmap="viridis")
plt.xlabel("UMAP 1")
plt.ylabel("UMAP 2")
plt.colorbar(label="powerplay position")
plt.title("UMAP embedding of curling shot-level board states")
plt.show()
```
```{python}
plt.figure(figsize=(10,8))
plt.scatter(df["umap_x"], df["umap_y"], alpha=0.7, c=df["task"],cmap="viridis")
plt.xlabel("UMAP 1")
plt.ylabel("UMAP 2")
