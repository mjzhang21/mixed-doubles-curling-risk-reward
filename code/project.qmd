...
```
```{python}
freq_table = df_hammer['steal'].value_counts(dropna=False).reset_index()
freq_table.columns = ['steal', 'Frequency']
freq_table['Percentage'] = (freq_table['Frequency'] / freq_table['Frequency'].sum() * 100).round(2)
print(freq_table)
bars = plt.bar(freq_table['steal'].astype(str), freq_table['Frequency'], color='skyblue')
...
stone_features = []
aggregate_features=[]
numeric_features = ['points','timeout','handle']
categorical_features = ["powerplay", "task"]

all_features = stone_features + aggregate_features + categorical_features + numeric_features
...
aggregate_features = ["num_stones_in_play", "r_min", "r_max", "r_mean"]
categorical_features = ["powerplay", "task"]
...
continuous_features = stone_features + aggregate_features + ["shot_id", "hammer"]
