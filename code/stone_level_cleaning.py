...
score_progress["PreEndScore"] = (
    score_progress.groupby(["MatchID", "TeamID"])["Result"] # chronological sort
    .transform(lambda s: s.cumsum().shift(1)) # shifts to prior score
    .fillna(0)
    .astype(int)
)
...
score_progress = score_progress.merge(
    ...
    .rename(columns={"TeamID": "OppTeamID", "PreEndScore": "OppPreEndScore"}),
    on=["MatchID", "EndID"],
    how="left",
    validate="many_to_many"
)
score_progress = score_progress[score_progress["TeamID"] != score_progress["OppTeamID"]]
...
stones_master = stones_master.merge(
    score_progress[["MatchID", "EndID", "TeamID", "PreEndScore", "OppPreEndScore", "PreEndScoreDiff"]],
    on=["MatchID", "EndID", "TeamID"],
    how="left",
    validate="many_to_one"
)
