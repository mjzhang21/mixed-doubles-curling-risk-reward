...
\section{Methodology}
\subsection{Power Play and Late-Phase Shot Selection - Methodology}

This analysis uses the master stone-level file
\texttt{stones\_master\_1.csv} and defines the unit of analysis as a
team-end (two rows per end, one per team). We restrict all behavioral
measurement to the late phase of the end (\texttt{ShotID} 16--22) to
avoid the rule-constrained early phase. Each late-phase shot is mapped
into a coarse behavioral class using the analyst task codes:
offensive (\texttt{Draw}, \texttt{Front}, \texttt{Guard},
\texttt{Raise}, \texttt{Freeze}, \texttt{Hit and roll}), defensive
(\texttt{Wick/soft peel}, \texttt{Take-out}, \texttt{Clearing},
\texttt{Double take-out}, \texttt{Promotion take-out}), and neutral
(\texttt{Through} or \texttt{No shot}). This operationalization treats
behavior as shot selection, which is the only directly observed
strategic signal in the data.

Power play exposure is defined at two levels. At the end level we set
\texttt{pp\_used\_end} to 1 if any team used the power play in that end.
At the team-end level we set \texttt{pp\_by\_team} to 1 if that team
called the power play. These are combined into a three-level indicator:
\texttt{nonPP} (no power play used), \texttt{PP\_caller} (team called the
power play), and \texttt{PP\_noncaller} (opponent of the caller). Each
team-end inherits \texttt{has\_hammer}, \texttt{pre\_end\_score\_diff},
\texttt{end\_id}, and team and match identifiers from the merged data.

For each team-end we compute late-phase counts of offensive, defensive,
and neutral shots and divide by the number of late shots observed to
form shares. The primary continuous outcome is the behavior index,
\texttt{behavior\_index} $=$ \texttt{def\_share\_late} $-$
\texttt{off\_share\_late}, where positive values indicate more defensive
shot selection. We also compute two timing splits: lateA (shots 16--18)
and lateB (shots 19--22). Because hammer teams throw four late shots and
non-hammer teams throw three, shares are in 1/4 or 1/3 increments; this
is expected and handled through share-based outcomes rather than raw
counts.

The core model is fixed-effects OLS with clustered inference:
\texttt{behavior\_index} $\sim$ \texttt{C(pp\_group)} +
\texttt{has\_hammer} + \texttt{pre\_end\_score\_diff} +
\texttt{C(end\_id)} + \texttt{C(team\_id)} + \texttt{C(match\_id)}.
Team and match fixed effects absorb persistent strategy differences and
match-level conditions, while clustering by match accounts for within-
match correlation. We fit identical models for \texttt{behavior\_index}
lateA and lateB. Diagnostics include residual summaries, residuals vs
fitted plots, Q--Q plots, and influence measures. As a validation (not a
behavior definition), we compute \texttt{opp\_offsheet\_late}, the number
of opponent stones that transition to the off-sheet sentinel (4095)
during late shots, using the fixed stone ownership (stones 1--6 for
\texttt{team\_id1}, stones 7--12 for \texttt{team\_id2}).


\subsection{First-shot Non-hammer Team Clustering}
...
\subsection{Power Play and Late-Phase Shot Selection - Results}

The late-phase sample contains 5,274 team-end rows (2,637 ends). Descriptively,
non-power-play ends are strongly offensive: mean
\texttt{def\_share\_late} is 0.295 while \texttt{off\_share\_late} is 0.690,
yielding a mean \texttt{behavior\_index} of $-0.395$ (median $-0.500$). Power
play ends shift toward defense for both teams. For PP callers,
\texttt{def\_share\_late} rises to 0.426 and \texttt{off\_share\_late} falls to
0.566, producing a mean \texttt{behavior\_index} of $-0.141$ (median 0.000).
For PP non-callers, \texttt{def\_share\_late} is 0.455 and
\texttt{off\_share\_late} is 0.540, with a mean \texttt{behavior\_index} of
$-0.085$ (median $-0.333$). Thus, both teams become more defensive in power
play ends, with the larger shift occurring for the opponent.

Score-state stratification shows that power play is rarely used when tied
(0.113) and is more common when trailing or leading (0.265 each). Within PP
ends, the caller becomes markedly more defensive when tied or leading
(\texttt{def\_share\_late} 0.503 tied; 0.658 leading), while the non-caller is
especially defensive when the caller is leading (\texttt{def\_share\_late}
0.587). Timing splits support an early shift for the caller and a sustained
shift for the non-caller: in lateA, defensive shares are 0.417 (caller) and
0.457 (non-caller) versus 0.266 in non-PP ends; in lateB they are 0.434 and
0.455 versus 0.314 in non-PP ends.

The fixed-effects regression confirms these patterns after controlling for
hammer, score differential, end, team, and match. Relative to non-PP ends, the
PP caller coefficient is 0.068 (SE 0.030, $p=0.024$) and the PP non-caller
coefficient is 0.449 (SE 0.035, $p\approx 1.3\times10^{-37}$) in the main
model ($R^2=0.232$). Timing models show the caller's effect concentrated in
lateA (0.128, SE 0.038, $p=8.5\times10^{-4}$) and absent in lateB (0.009,
SE 0.038, $p=0.82$), whereas the non-caller remains strongly defensive in both
lateA (0.516, SE 0.046, $p\approx 1.3\times10^{-29}$) and lateB (0.414,
SE 0.043, $p\approx 3.1\times10^{-22}$). Diagnostics show no single
observation dominates the fit (max Cook's distance 0.0025; max leverage 0.10),
and residuals are centered with moderate dispersion (SD $\approx$ 0.55).

The positional validation supports the behavioral interpretation: the Spearman
correlation between \texttt{def\_share\_late} and opponent stones removed late
is 0.804. Together, the descriptive and controlled results indicate that power
play ends induce a defensive shift for both teams, with a substantially larger
and more persistent response from the non-caller.
...
PCA, t-SNE and UMAP revealed well separated clusters with minimal
overlap.(Figure~\ref{fig:fs_tSNE}) With noise excluded, silhouette score was
