...
def mean_ci(data, alpha=0.05):
    ...
    h = se * t_dist.ppf(1 - alpha/2, n - 1)
    return mean_val, mean_val - h, mean_val + h

# ----------------------------
# Pairwise Welch post-hoc
# ----------------------------
def pairwise_welch(df, var, group_var, groups):
    results = []

    for g1, g2 in combinations(groups, 2):
        x1 = df[df[group_var] == g1][var].values
        x2 = df[df[group_var] == g2][var].values
        stat, p = ttest_ind(x1, x2, equal_var=False)
        results.append((g1, g2, p))

    raw_p = [r[2] for r in results]
    _, p_adj, _, _ = multipletests(raw_p, method="holm")

    print("Pairwise Welch t-tests (Holm-adjusted):")
    for (g1, g2, _), p in zip(results, p_adj):
        print(f"{g1} vs {g2}: p={p:.4f}")

# ----------------------------
# Main analysis function
# ----------------------------
def analyze_variable(df, var, group_var, groups_to_use, method="kw", dunn_adjust="holm"):
    df_var = df[df[group_var].isin(groups_to_use)][[group_var, var]].dropna()
    print(f"\n=== Variable: {var} grouped by {group_var} ({method}) ===")
    ...
    for g in groups_to_use:
        ...
        if method == "kw":
            med, q1, q3 = median_iqr(data)
            print(f"{group_var} {g}: median={med:.2f}, IQR=({q1:.2f}, {q3:.2f}), n={n}")
        elif method == "welch":
            mean_val, lower, upper = mean_ci(data)
            print(f"{group_var} {g}: mean={mean_val:.2f}, 95% CI=({lower:.2f}, {upper:.2f}), n={n}")

    # ----------------------------
    # INFERENCE
    # ----------------------------
    if method == "kw":
        groups = [df_var[df_var[group_var] == g][var].values for g in groups_to_use]
        kw_stat, kw_p = kruskal(*groups)
        print(f"Kruskal–Wallis H={kw_stat:.3f}, p={kw_p:.3f}")

        if kw_p <= 0.05 and len(groups_to_use) > 1:
            print("Pairwise comparisons (Dunn test):")
            dunn = sp.posthoc_dunn(
                df_var,
                val_col=var,
                group_col=group_var,
                p_adjust=dunn_adjust
            )
            print(dunn)
        ...
        print(f"Welch ANOVA p={pval:.3f}")

        if pval <= 0.05 and len(groups_to_use) > 1:
            pairwise_welch(df_var, var, group_var, groups_to_use)

```

```{python}
group_var = 'hdbscan_cluster'
groups_to_use = [0,1,3]  # choose which groups to include
variables = ['cumulative_score_diff','end_score_diff','handle']

for var in variables:
    analyze_variable(df_pp2, var, group_var, groups_to_use,method="welch")
...
for var in variables:
    analyze_variable(df_pp2, var, group_var, groups_to_use,method="kw")
...
for var in variables:
    analyze_variable(df_pp, var, group_var, groups_to_use,method="welch")
...
for var in variables:
    analyze_variable(df_pp, var, group_var, groups_to_use,method="kw")
