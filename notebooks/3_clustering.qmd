...
def bootstrap_mean_ci(data, n_boot=10000, ci=95):
    ...
    lower, upper = bs.conf_int(np.mean, reps=n_boot, method='bca')
...
# ----------------------------
# Analyze variable function (updated)
# ----------------------------
def analyze_variable(df, var, group_var, groups_to_use, use_bootstrap=True, dunn_adjust='holm'):
    df_var = df[df[group_var].isin(groups_to_use)][[group_var, var]].dropna()
    print(f"\n=== Variable: {var} grouped by {group_var} ===")

    # Descriptive CI
    for g in groups_to_use:
        data = df_var[df_var[group_var]==g][var].values
        if use_bootstrap:
            mean_val, lower, upper = bootstrap_mean_ci(data)
        else:
            mean_val, lower, upper = mean_ci(data)
        print(f"{group_var} {g}: mean={mean_val:.2f}, 95% CI=({lower:.2f}, {upper:.2f}), n={len(data)}")

    # If using bootstrap → Kruskal-Wallis + Dunn
    if use_bootstrap:
        # Kruskal-Wallis
        ...
        if kw_p <= 0.05 and len(groups_to_use) > 1:
            ...
            dunn_results = sp.posthoc_dunn(df_var, val_col=var, group_col=group_var, p_adjust=dunn_adjust)
            print(dunn_results)
    else:
        # Welch ANOVA
        ...
        welch_p = anova_res['PR(>F)'][f'C({group_var})']
        print(f"Welch ANOVA p={welch_p:.3f}")
```

```{python}
group_var = 'hdbscan_cluster'
groups_to_use = [0,1,3]  # choose which groups to include
variables = ['cumulative_score_diff','end_score_diff','nonhammer_win','hammer_2plus','handle']
