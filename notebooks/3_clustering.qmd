...
for cluster_id in sorted(df_pp2['hdbscan_cluster'].dropna().unique()):
    ...
    ax.set_title(f"Cluster {cluster_id} | Powerplay=2")
    ax.legend()
    plt.show()
...
# ----------------------------
def mean_ci(mean, std, n, alpha=0.05):
    """Return mean and 95% CI. Handles n < 2."""
    if n < 2:
...
# Main function
# ----------------------------
def forest_plot_summary(df, group_var, metrics, exclude=None):
    """
    Compute mean ± 95% CI for metrics grouped by group_var and plot forest plots.
    
    Parameters:
        df : pd.DataFrame
        group_var : str, column to group by (e.g., 'task' or 'hdbscan_cluster')
        metrics : list of str, columns to summarize and plot
        exclude : list, optional, group values to exclude
    Returns:
        cluster_summary_ci : pd.DataFrame with mean, lower, upper for all metrics
    """
    # Filter excluded groups
    if exclude is not None:
        df = df[~df[group_var].isin(exclude)]
    
    # Compute summary
    summary_list = []
    for group_id, group in df.groupby(group_var):
        d = {group_var: group_id, 'n': len(group)}
        for var in metrics:
            mean = group[var].mean()
            std = group[var].std(ddof=1)
            m, lower, upper = mean_ci(mean, std, len(group))
            d[f'{var}_mean'] = m
            d[f'{var}_lower'] = lower
            d[f'{var}_upper'] = upper
        summary_list.append(d)
    
    summary_df = pd.DataFrame(summary_list).sort_values(group_var).reset_index(drop=True)
    print(summary_df)
    
    # ----------------------------
    # Minimal forest plots
    # ----------------------------
    groups = summary_df[group_var].astype(str)
    y_pos = np.arange(len(groups))
    
    for var in metrics:
        means = summary_df[f'{var}_mean']
        lower = summary_df[f'{var}_lower']
        upper = summary_df[f'{var}_upper']
        
        xerr = np.vstack([means - lower, upper - means])
        
        fig, ax = plt.subplots(figsize=(6, len(groups)*0.5 + 1))
        ax.errorbar(means, y_pos, xerr=xerr, fmt='o', color='black', capsize=3)
        
        # Reference line for difference metrics
        if var in ['cumulative_score_diff', 'end_score_diff']:
...
cluster_summary = forest_plot_summary(df_pp2[df_pp2['hdbscan_cluster'] != -1],
                                      'hdbscan_cluster', metrics)
...
task_summary = forest_plot_summary(df_pp2, 'task', metrics, exclude=[5, 10])
```
...
task_summary = forest_plot_summary(df_pp, 'task', metrics,[5, 6,8,10])
```
...
# Only keep countries with ≥20 shots
df_filtered = df_filtered.groupby('noc').filter(lambda x: len(x) >= 25)

# Calculate proportions per country for each task
prop_table = df_filtered.groupby(['noc', 'task']).size().unstack(fill_value=0)

# Convert to proportion within the country
prop_table = prop_table.div(prop_table.sum(axis=1), axis=0)

# For each task, sort countries by proportion
for task in prop_table.columns:
    top_countries = prop_table[task].sort_values(ascending=False)
    print(f"\nTask {task} - countries with highest proportion of shots:")
    print(top_countries.head(5))  # show top 5 countries

```
```{python}
df_filtered['noc'].value_counts()
```
...
model = ols('points ~ C(task)', data=df_filtered).fit()

# Welch ANOVA (robust to unequal variances)
anova_results = anova_lm(model, typ=2, robust='hc3')
print("Welch ANOVA results for points:")
print(anova_results)

# Post-hoc pairwise comparisons (Tukey-Kramer)
tukey = pairwise_tukeyhsd(endog=df_filtered['points'], groups=df_filtered['task'], alpha=0.05)
print("Tukey-Kramer post-hoc for points:")
print(tukey.summary())
```

```{python}
df_filter = df_filtered[['task','cumulative_score_diff']].dropna()
model = ols('cumulative_score_diff ~ C(task)', data=df_filter).fit()

# Welch ANOVA (robust to unequal variances)
anova_results = anova_lm(model, typ=2, robust='hc3')
print("Welch ANOVA results for cumulative_score_diff:")
print(anova_results)

# Post-hoc pairwise comparisons (Tukey-Kramer)
tukey = pairwise_tukeyhsd(endog=df_filter['cumulative_score_diff'], groups=df_filter['task'], alpha=0.05)
print("Tukey-Kramer post-hoc for cumulative_score_diff:")
print(tukey.summary())
...
```{python}
model = ols('score_nonhammer ~ C(task)', data=df_filtered).fit()
...
# Post-hoc pairwise comparisons (Tukey-Kramer)
tukey = pairwise_tukeyhsd(endog=df_filtered['score_nonhammer'], groups=df_filtered['task'], alpha=0.05)
print("Tukey-Kramer post-hoc for points:")
print(tukey.summary())
...
```{python}
model = ols('end_score_diff ~ C(task)', data=df_filtered).fit()
...
contingency = pd.crosstab(df_filtered['task'], df_filtered['nonhammer_win'])
chi2, p, dof, expected = chi2_contingency(contingency)
print(f"Chi-square test for nonhammer_win by task: chi2={chi2:.2f}, p={p:.4f}")
```
...
# Create contingency table
contingency = pd.crosstab(df_filtered['task'], df_filtered['hammer_2plus'])
chi2, p, dof, expected = chi2_contingency(contingency)
print(f"Chi-square test for nonhammer_win by task: chi2={chi2:.2f}, p={p:.4f}")
```
