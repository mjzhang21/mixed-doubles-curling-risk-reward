...
```
```{python}
df_pp2_na = df_pp2[df_pp2[cols].isna().any(axis=1)]
...
```{python}
df_pp2 = df_pp2.merge(
    end_level[['match_id','end_id','cumulative_score_diff',
...
s1x = df_pp2['stone_1_x'].copy()
s1y = df_pp2['stone_1_y'].copy()
s7x = df_pp2['stone_7_x'].copy()
s7y = df_pp2['stone_7_y'].copy()

# Masks for sentinel positions
mask_1 = (s1x == 350) & (s1y == 850)
mask_7 = (s7x == 411) & (s7y == 1916)

# Combine: swap if EITHER condition holds
swap_mask = mask_1 | mask_7

# Swap coordinates
df_pp2.loc[swap_mask, 'stone_1_x'] = s7x[swap_mask]
df_pp2.loc[swap_mask, 'stone_1_y'] = s7y[swap_mask]
df_pp2.loc[swap_mask, 'stone_7_x'] = s1x[swap_mask]
df_pp2.loc[swap_mask, 'stone_7_y'] = s1y[swap_mask]

# How many rows were swapped
swap_mask.sum()
```
...
import seaborn as sns
import matplotlib.pyplot as plt
df_pp2['nonhammer_win'] = (df_pp2['result']>0).astype(int)
...
cluster_summary = (
    ...
    .agg(
        ...
        avg_weighted_score_diff=('weighted_score_diff', 'mean'),
        n=('nonhammer_win', 'count')
    )
)
