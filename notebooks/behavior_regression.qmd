...
This bar chart compares mid-window versus closing-window defensive share within
each PP group to provide a timing mechanism check.

```{python}
# 3) Timing split: mid vs close defensive share by pp_group
mean_timing = (
    team_end.groupby(["pp_group"], as_index=False)
    .agg(
        def_share_mid=("def_share_mid", "mean"),
        def_share_close=("def_share_close", "mean"),
    )
)
mean_timing = mean_timing.melt(
    id_vars=["pp_group"],
    value_vars=["def_share_mid", "def_share_close"],
    var_name="phase",
...
plot_timing = (
    ...
    + labs(
        title="Late-phase defensive share: mid vs close",
        x="PP group",
        y="Defensive share",
        fill="Phase",
    )
