...
context, and build the three-level power-play exposure variable. We also create score
bins from pre-end score differential for descriptive stratification.

The end-goal here is to identify behavior and its changes in various game-states while
*controlling* / attributing *importance* to these various gamestates (e.g., hammer
possession, score differential, etc.), making the data fed into the model incredibly
important. Each end contains the same context for both teams, but the `team_end`
observational unit allows modelling nuance not available to pure end-level data.

Behavior in curling is not mutually informative, i.e., one team's behavior does not
necessarily dictate another's. This implies that for modelling to capture every team's
behavior by their shot-selection using every end, we need an observational unit that
captures the aforementioned behavior for every team using every end. Hence the team-end.

It is possible to use end-level data by containing both team's behavior in a single
row for each end, but this would counterproductively add complexity to the model and
would make a TeamID fixed effect impossible. 

Obviously, each team-end comes in a pair where everything will be the same except,
of-course, behavior.
...
are those that have no rules. I do a sub-split of the data into categories
`lateA` and `lateB` intended to capture the change in behavior (elaborated
upon later). Essentially, these are supposed to represent the early
and late throwing windows respectievly.
This is a pragamatic timing split intended to isolate closing behavior,
which I consider likely to differ from mid-end behavior which might still be
offensively-skewed.
