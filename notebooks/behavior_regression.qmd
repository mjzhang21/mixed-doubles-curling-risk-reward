...
- Unit: team-end (two rows per end). Treat rows as paired, not independent. -
  Phase restriction: late phase only, shot_id 16-22. - Behavior labels (shot
  selection only): - Defensive = opponent-addressing (takeouts, peels,
  clears). - Offensive = placement or pressure (draws, guards, raises). -
  Neutral/unknown = through, no-shot, or unknown.

## Setup

We load the core libraries for data manipulation, plotting, and regression. The
plotnine and statsmodels imports are required for the figures and inference;
the notebook will fail fast if they are missing so you know exactly what needs
to be installed.
...
try:
    ...
    raise ImportError("plotnine is required for plots.") from exc

try:
    import statsmodels.formula.api as smf
except ImportError as exc:
    raise ImportError("statsmodels is required for regression models.") from exc

try:
    from scipy import stats
except ImportError as exc:
    raise ImportError("scipy is required for diagnostics.") from exc
```

## Load data

We run the stone-level cleaning script directly to construct the master table
in memory, which keeps the analysis reproducible without relying on a stored
CSV. After loading the cleaned dataframe, we standardize the key columns used
throughout the analysis. Tasks are coerced to integers and missing tasks are
set to -1 (neutral/unknown), while other fields are converted to integers
where appropriate. The preview confirms the core identifiers and fields were
loaded correctly.
