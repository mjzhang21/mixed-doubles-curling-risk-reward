...
# Filter nations with at least 20 shots
valid_nations = df_filtered['noc'].value_counts()
valid_nations = valid_nations[valid_nations >= 20].index
agg_task_dict = {name: data.loc[data.index.isin(valid_nations)] for name, data in agg_task_dict.items()}

# Get all nations that appear in any top 5
all_nations = set()
...
palette = sns.color_palette("tab20", n_colors=len(all_nations))  # tab20 has up to 20 distinct colors
nation_colors = dict(zip(all_nations, palette))

# Plot
fig, axes = plt.subplots(1, len(agg_task_dict), figsize=(12, 6), sharey=False)

for i, (group_name, data) in enumerate(agg_task_dict.items()):
    ax = axes[i]

    # Top 5 nations for this group
    data_top = data.sort_values(ascending=True)[-5:]

    # Bars with consistent colors
    colors = [nation_colors[n] for n in data_top.index]
    ax.barh(data_top.index, data_top.values, color=colors)
...
plt.savefig("noc_pptask.pdf", format="pdf", bbox_inches="tight")
plt.show()
