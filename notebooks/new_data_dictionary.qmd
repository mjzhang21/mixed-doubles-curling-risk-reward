...
*This data dictionary is for the master working dataframe* `stones_master_1.csv`.
*Some notes about data quirks are also included.*
...
- `GroupID` - In competitions where teams are split into group for play, the ID
...
- `TeamID1` – ID for the team corresponding to NOC1 and ResultsStr1.
- `TeamID2` - ID for the team corresponding to NOC2 and ResultsStr2.
...
of 1 corresponds to pre-palced stones being moved to the right side, 2 being pre-placed stones are moved to the left side. 0 means the team is not using their
powerplay.
- `HammerTeamID_C` - Which `TeamID` in the end has the hammer.
- `HasHammer_C` - Boolean for whether the skipping team has the hammer.

#### Shot-By-Shot Data

- `ShotID` - ID for the shot. Within a particular match and end, shots occur in
